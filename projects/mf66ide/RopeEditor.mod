...
VAR
  ...
  gHoverSet: BOOLEAN;
  gEnterProc: EnterProc;        (* Return-key hook (TRUE = handled, no newline) *)
  gEnterSet: BOOLEAN;
  gLexMode: CARDINAL;           (* 0 = Forth, 1 = masm (per active editor) *)
...
(* a word that OPENS a string: ends with `"` (`."`, `s"`, `c"`, `abort"`, `s\"`). *)
PROCEDURE EndsQuote (w: ARRAY OF CHAR): BOOLEAN;
VAR L: CARDINAL;
BEGIN L := Length(w); RETURN (L >= 2) AND (w[L-1] = '"') END EndsQuote;

(* a Forth number: integer (opt `-`), `$hex`, or a float (digits `.` digits opt `e`).
   Words that merely START with a digit (`2dup`, `0=`, `0<`) are NOT numbers. *)
PROCEDURE IsFNumber (w: ARRAY OF CHAR): BOOLEAN;
VAR i, L, digits: CARDINAL; c: CHAR; dot, exp: BOOLEAN;
BEGIN
  L := Length(w);
  ...
  IF w[i] = '$' THEN                                   (* $hex *)
    INC(i); IF i >= L THEN RETURN FALSE END;
    WHILE i < L DO c := w[i];
      IF NOT (IsDigit(c) OR ((c>='a')AND(c<='f')) OR ((c>='A')AND(c<='F'))) THEN RETURN FALSE END;
      INC(i) END;
    RETURN TRUE
  END;
  digits := 0; dot := FALSE; exp := FALSE;
  WHILE i < L DO
    c := w[i];
    IF IsDigit(c) THEN INC(digits)
    ELSIF (c = '.') AND (NOT dot) AND (NOT exp) THEN dot := TRUE
    ELSIF ((c = 'e') OR (c = 'E')) AND (NOT exp) AND (digits > 0) THEN
      exp := TRUE;
      IF (i+1 < L) AND ((w[i+1]='+') OR (w[i+1]='-')) THEN INC(i) END
...
(* Forth control / defining words (case-insensitive). Stack/arith words stay plain
   so structure stands out. *)
PROCEDURE IsKeyword (w: ARRAY OF CHAR): BOOLEAN;
VAR i: CARDINAL;
BEGIN
  i := 0;
  WHILE w[i] # CHR(0) DO IF (w[i] >= 'A') AND (w[i] <= 'Z') THEN w[i] := CHR(ORD(w[i])+32) END; INC(i) END;
  RETURN Equal(w,":") OR Equal(w,";") OR Equal(w,":noname") OR Equal(w,"does>") OR
    Equal(w,"create") OR Equal(w,"variable") OR Equal(w,"constant") OR Equal(w,"value") OR
    Equal(w,"2value") OR Equal(w,"fvalue") OR Equal(w,"fconstant") OR Equal(w,"fvariable") OR
    Equal(w,"to") OR Equal(w,"defer") OR Equal(w,"is") OR Equal(w,"immediate") OR
    Equal(w,"code") OR Equal(w,"end-code") OR Equal(w,"recurse") OR Equal(w,"recursive") OR
    Equal(w,"postpone") OR Equal(w,"literal") OR Equal(w,"[char]") OR Equal(w,"[']") OR
    Equal(w,"[") OR Equal(w,"]") OR Equal(w,"'") OR
    Equal(w,"if") OR Equal(w,"then") OR Equal(w,"else") OR
    Equal(w,"begin") OR Equal(w,"until") OR Equal(w,"while") OR Equal(w,"repeat") OR Equal(w,"again") OR
    Equal(w,"do") OR Equal(w,"?do") OR Equal(w,"loop") OR Equal(w,"+loop") OR
    Equal(w,"leave") OR Equal(w,"unloop") OR Equal(w,"i") OR Equal(w,"j") OR
    Equal(w,"case") OR Equal(w,"of") OR Equal(w,"endof") OR Equal(w,"endcase") OR
    Equal(w,"exit")
END IsKeyword;
...
BEGIN RETURN (c = ' ') OR (c = CHR(9)) OR (c = CHR(10)) OR (c = CHR(13)) END IsSpace;

PROCEDURE LexForth (text: ARRAY OF CHAR; n: CARDINAL; p: PRuns);
VAR i, j, k: CARDINAL; w: ARRAY [0..63] OF CHAR;
BEGIN
  p^.count := 0; i := 0;
  WHILE i < n DO
    IF IsSpace(text[i]) THEN
      AddRun(p, 1, kDefault); INC(i)
    ELSIF (text[i] = CHR(92)) AND ((i+1 >= n) OR IsSpace(text[i+1])) THEN
      (* `\` line comment to end of line *)
      j := i; WHILE (j < n) AND (text[j] # CHR(10)) DO INC(j) END;
      AddRun(p, j-i, kComment); i := j
    ELSIF (text[i] = '(') AND (i+1 < n) AND IsSpace(text[i+1]) THEN
      (* `( ... )` inline comment *)
      j := i+1; WHILE (j < n) AND (text[j] # ')') DO INC(j) END;
      IF j < n THEN INC(j) END;
      AddRun(p, j-i, kComment); i := j
    ELSE
      (* a whitespace-delimited Forth word *)
      j := i;
      WHILE (j < n) AND NOT IsSpace(text[j]) DO INC(j) END;
      k := 0; WHILE (i+k < j) AND (k < 63) DO w[k] := text[i+k]; INC(k) END; w[k] := CHR(0);
      IF EndsQuote(w) THEN
        (* string opener `." … "` / `s" … "`: run to the closing quote *)
        WHILE (j < n) AND (text[j] # '"') AND (text[j] # CHR(10)) DO INC(j) END;
        IF (j < n) AND (text[j] = '"') THEN INC(j) END;
        AddRun(p, j-i, kString); i := j
      ELSIF IsFNumber(w) THEN AddRun(p, j-i, kNumber); i := j
      ELSIF IsKeyword(w) THEN AddRun(p, j-i, kKeyword); i := j
      ELSE AddRun(p, j-i, kDefault); i := j
      END
    END
  END
END LexForth;

(* ── JASM .masm lexer: `;` line comments, "…"/'…' strings, numbers (0x.., #imm),
   `@`-directives + proc/endp/next/… keywords; mnemonics/labels stay plain. ── *)
PROCEDURE IsMasmIdent (c: CHAR): BOOLEAN;
BEGIN RETURN IsAlpha(c) OR IsDigit(c) OR (c='@') OR (c='.') OR (c='$') END IsMasmIdent;

PROCEDURE IsMasmKw (w: ARRAY OF CHAR): BOOLEAN;
VAR i: CARDINAL;
BEGIN
  IF w[0] = '@' THEN RETURN TRUE END;              (* @assign / @scope / @extern / … *)
  i := 0;
  WHILE w[i] # CHR(0) DO IF (w[i] >= 'A') AND (w[i] <= 'Z') THEN w[i] := CHR(ORD(w[i])+32) END; INC(i) END;
  RETURN Equal(w,"proc") OR Equal(w,"endp") OR Equal(w,"next") OR Equal(w,"stk") OR
    Equal(w,"macro") OR Equal(w,"endm") OR Equal(w,"equ") OR Equal(w,"align") OR
    Equal(w,"db") OR Equal(w,"dw") OR Equal(w,"dd") OR Equal(w,"dq") OR
    Equal(w,"byte") OR Equal(w,"word") OR Equal(w,"dword") OR Equal(w,"qword") OR
    Equal(w,"ptr") OR Equal(w,"global") OR Equal(w,"globl") OR Equal(w,"extern") OR
    Equal(w,"export") OR Equal(w,"section") OR Equal(w,"include")
END IsMasmKw;

PROCEDURE LexMasm (text: ARRAY OF CHAR; n: CARDINAL; p: PRuns);
VAR i, j, k: CARDINAL; w: ARRAY [0..63] OF CHAR;
BEGIN
  p^.count := 0; i := 0;
  WHILE i < n DO
    IF text[i] = ';' THEN                                   (* ; comment to EOL *)
      j := i; WHILE (j < n) AND (text[j] # CHR(10)) DO INC(j) END;
      AddRun(p, j-i, kComment); i := j
    ELSIF (text[i] = '"') OR (text[i] = "'") THEN           (* string / char literal *)
      k := ORD(text[i]); j := i+1;
      WHILE (j < n) AND (ORD(text[j]) # k) AND (text[j] # CHR(10)) DO INC(j) END;
      IF (j < n) AND (ORD(text[j]) = k) THEN INC(j) END;
      AddRun(p, j-i, kString); i := j
    ELSIF IsDigit(text[i]) OR ((text[i] = '#') AND (i+1 < n) AND IsDigit(text[i+1])) THEN
      j := i+1; WHILE (j < n) AND (IsAlpha(text[j]) OR IsDigit(text[j])) DO INC(j) END;
      AddRun(p, j-i, kNumber); i := j
    ELSIF IsAlpha(text[i]) OR (text[i]='@') OR (text[i]='.') OR (text[i]='_') THEN
      j := i; WHILE (j < n) AND (IsMasmIdent(text[j]) OR (text[j]='_')) DO INC(j) END;
      k := 0; WHILE (i+k < j) AND (k < 63) DO w[k] := text[i+k]; INC(k) END; w[k] := CHR(0);
      IF IsMasmKw(w) THEN AddRun(p, j-i, kKeyword) ELSE AddRun(p, j-i, kDefault) END;
      i := j
    ELSE
      AddRun(p, 1, kDefault); INC(i)
    END
...
(* Dispatcher: the active editor's mode (SetLexMode) picks the lexer. *)
PROCEDURE Lex (text: ARRAY OF CHAR; n: CARDINAL; p: PRuns);
BEGIN
  IF gLexMode = 1 THEN LexMasm(text, n, p) ELSE LexForth(text, n, p) END
END Lex;
...
CLASS RopeTextView;
  ...
  BEGIN
    ...
    IF gEnterSet AND gEnterProc(me) THEN RETURN END;   (* console pane handled Return *)
...
PROCEDURE SetEnterProc (p: EnterProc);
BEGIN gEnterProc := p; gEnterSet := TRUE END SetEnterProc;

PROCEDURE SetLexMode (mode: CARDINAL);
BEGIN gLexMode := mode END SetLexMode;
...
BEGIN
  ...
  (* a CODE editor: keep symbols literal. Smart quotes would turn a typed " into a
     curly “ ” (breaking `." …"` / `s" …"`), smart dashes turn -- into —, etc. *)
  [tvId setAutomaticQuoteSubstitutionEnabled: FALSE];
  [tvId setAutomaticDashSubstitutionEnabled: FALSE];
  [tvId setAutomaticTextReplacementEnabled: FALSE];
  [tvId setAutomaticSpellingCorrectionEnabled: FALSE];
  [tvId setSmartInsertDeleteEnabled: FALSE];
  [tvId setContinuousSpellCheckingEnabled: FALSE];
  [tvId setGrammarCheckingEnabled: FALSE];
  [tvId setAutomaticLinkDetectionEnabled: FALSE];
  [tvId setAutomaticDataDetectionEnabled: FALSE];
...
BEGIN
  gInited := FALSE; gHoverSet := FALSE; gEnterSet := FALSE; gLexMode := 0
END RopeEditor.
