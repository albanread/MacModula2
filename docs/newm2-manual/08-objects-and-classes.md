...
modules — see [Modules & compilation](03-modules-and-compilation.md).
...
See [Memory & exceptions](10-memory-and-exceptions.md) for the allocator and the
...
IID), and a native class and an interface can't be mixed in one `GUARD`/`ISMEMBER`. Qualified
interface names (`Mod.IFoo`) work in both.
...
---
*See also [Declarations & types](04-declarations-and-types.md),
[Memory & exceptions](10-memory-and-exceptions.md), and the design note
