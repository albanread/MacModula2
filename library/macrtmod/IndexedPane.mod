...
PROCEDURE SetRGB (index, r, g, b: CARDINAL);
  VAR o: CARDINAL;
BEGIN
  IF (index < 16) OR (index > 255) THEN RETURN END;
  o := (PH*16 + (index-16)) * 4;
...
(* a 5x7 digit font (MSB-left); letters render blank for now *)
VAR fontD: ARRAY [0..69] OF BYTE; fontReady: BOOLEAN;
PROCEDURE FRow (g, row, bits: CARDINAL);
BEGIN fontD[g*7 + row] := VAL(BYTE, bits) END FRow;
PROCEDURE SeedFont;
BEGIN
  fontReady := TRUE;
  FRow(0,0,0EH);FRow(0,1,11H);FRow(0,2,11H);FRow(0,3,11H);FRow(0,4,11H);FRow(0,5,11H);FRow(0,6,0EH);
  FRow(1,0,04H);FRow(1,1,0CH);FRow(1,2,04H);FRow(1,3,04H);FRow(1,4,04H);FRow(1,5,04H);FRow(1,6,0EH);
  FRow(2,0,0EH);FRow(2,1,11H);FRow(2,2,01H);FRow(2,3,02H);FRow(2,4,04H);FRow(2,5,08H);FRow(2,6,1FH);
  FRow(3,0,1FH);FRow(3,1,02H);FRow(3,2,04H);FRow(3,3,02H);FRow(3,4,01H);FRow(3,5,11H);FRow(3,6,0EH);
  FRow(4,0,02H);FRow(4,1,06H);FRow(4,2,0AH);FRow(4,3,12H);FRow(4,4,1FH);FRow(4,5,02H);FRow(4,6,02H);
  FRow(5,0,1FH);FRow(5,1,10H);FRow(5,2,1EH);FRow(5,3,01H);FRow(5,4,01H);FRow(5,5,11H);FRow(5,6,0EH);
  FRow(6,0,06H);FRow(6,1,08H);FRow(6,2,10H);FRow(6,3,1EH);FRow(6,4,11H);FRow(6,5,11H);FRow(6,6,0EH);
  FRow(7,0,1FH);FRow(7,1,01H);FRow(7,2,02H);FRow(7,3,04H);FRow(7,4,08H);FRow(7,5,08H);FRow(7,6,08H);
  FRow(8,0,0EH);FRow(8,1,11H);FRow(8,2,11H);FRow(8,3,0EH);FRow(8,4,11H);FRow(8,5,11H);FRow(8,6,0EH);
  FRow(9,0,0EH);FRow(9,1,11H);FRow(9,2,11H);FRow(9,3,0FH);FRow(9,4,01H);FRow(9,5,02H);FRow(9,6,0CH)
END SeedFont;

PROCEDURE Text (x, y: INTEGER; s: ARRAY OF CHAR; index: CARDINAL);
  VAR i, g, row, col: CARDINAL; cx: INTEGER; ch: CHAR;
BEGIN
  IF NOT fontReady THEN SeedFont END;
  i := 0; cx := x;
  WHILE (i <= HIGH(s)) AND (s[i] # 0C) DO
    ch := s[i];
    IF (ch >= '0') AND (ch <= '9') THEN
      g := ORD(ch) - ORD('0');
      FOR row := 0 TO 6 DO FOR col := 0 TO 4 DO
        IF (VAL(CARDINAL, fontD[g*7+row]) DIV pow2(4-col)) MOD 2 = 1 THEN
