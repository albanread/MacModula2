...
BEGIN
  ...
  FOR i := 0 TO s.count-1 DO
    ...
    off := off + 2
  END;
  ok := Proc.WriteBytes(path, a, total) = 0;     (* single binary write *)
  DEALLOCATE(a, total);
  RETURN ok
END WriteWav;

PROCEDURE ReadWav (path: ARRAY OF CHAR; VAR s: Sound): BOOLEAN;
  VAR b: PByteBuf; a, sa: ADDRESS;
      fsize, got, pos, chunkSize, fmtOff, dataOff, dataLen: CARDINAL;
      tag, channels, sr, bits, bytesPerSample, n, i: CARDINAL;
      haveFmt, haveData: BOOLEAN; val, rc: INTEGER; c0, c1, c2: CARDINAL;
BEGIN
  rc := Proc.FileSize(path);
  IF rc < 44 THEN RETURN FALSE END;
  fsize := VAL(CARDINAL, rc);
  IF fsize > MaxWavBytes THEN RETURN FALSE END;
  ALLOCATE(a, fsize); IF a = NIL THEN RETURN FALSE END;
  b := CAST(PByteBuf, a);
  rc := Proc.ReadBytes(path, a, fsize);
  IF rc < 44 THEN DEALLOCATE(a, fsize); RETURN FALSE END;
  got := VAL(CARDINAL, rc);
  (* 'RIFF' .... 'WAVE' *)
